...
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
  ...
  theme(legend.position = "none")
```
There is a negative quadratic response in column one.
Numerically, n = 0 and k = 60 has a higher numerical mean but does mean that it the best agronomically.
...
summary(rcbd_mod)
```
with the restriction being applied, rep 4 is set zero, and the treatment 3 for both N and K is also set to zero.

All variable brhind the ~ are the treatment variable, main effects
...
# f) ANOVA table  
The `Anova()` function allows us to use **type 3 sum of squares**. (so that all three variables can be considered: rep, n_rate and k_rate_kgha) 
...
```
We use alpha value of 5% = 0.05, any number less than 0.05 are statistically significant.
...
Residuals are everything that remains after the msin effect, model are based on residuals.
...
Better to chech these assumptions, test visually, other than using other tests.
variance error ~ eiidN(0, sigma2), this is the sentence that breaks up the residuals.
...
3 and -3 are the standard number of deviation, assuming that 90% of our data are falling in this range.
...
- For this, we use the **fitted vs. residual plot**.  
- What we want to see: no clear pattern in residuals, random cloud of points.  
- What we do not want to see: residuals increasing as fitted value increases (fan shape).(funnel, increase in both the  - side and the + side)  
- Adding a `geom_smooth()` helps with that conclusion. We want to see the smooth line and error bands comprising 0 on the y axis.  
...
In case of an outlier that is outside the limit, 
...
ggplot() + 
  ...
  labs(x = "N rate + K rate (kh/ha)",
       y = "Yield (kg/ha") +
  scale_fill_viridis_d() +
  scale_color_viridis_d() +
  facet_grid(~nrate_kgha)+
  #scale_color_manual("red", "yellow"  )
  theme(legend.position = "none")
